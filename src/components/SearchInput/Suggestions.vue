        ...
        <n-scrollbar style="max-height: 45vh">
          ...
          <div class="all-result">
            ...
            >
              ...
              <span class="text">{{ item }}</span>
              <span class="delete" @click.stop="deleteHistoryItem(index)">✕</span>
            </div>
          </div>
        </n-scrollbar>
      </div>
...
const deleteHistoryItem = (index) => {
  site.searchHistory.splice(index, 1);
};
...
const keywordsSearch = debounce((val) => {
  ...
  if (searchKeyword.value) {
    // 调用搜索建议
    getSearchSuggestions(searchValue, set.suggestionProvider)
      .then((res) => {
        // 写入结果
